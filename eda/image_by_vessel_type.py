...
def get_traj(df,ID,min_length,seq_id,date):
    ...
    return print("{} created {} trajectories in {}".format(str(date),traj_count,datetime.now() - t_start)) #trajectories
...
for i in range(1,cnn_boats.index.days_in_month[0]+1): # loop by days in month
    ...
    get_traj(df=cnn_boats_in,ID='mmsi',min_length = 2000,seq_id = 0,date = date)
